...
const NOT_IN_VALIDATE: readonly { command: string; reason: string }[] = [
  ...
  },
  {
    ...
    reason:
      'Exercises the Postgres dialect against a live PostgreSQL service. The step loops over ' +
      'packages/core/src/db/*.postgres.integration.test.ts, so a new parity test is run by ' +
      'having been written, and each file gets its own process because each replaces the ' +
