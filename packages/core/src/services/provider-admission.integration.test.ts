...
// Each case points ARCHON_HOME at a fresh temp directory and expects its own
// empty slot table. Inside a container `getArchonHome()` ignores that and
// returns /.archon, so every case shared one database and saw the others' holders.
honorArchonHomeEnv();
