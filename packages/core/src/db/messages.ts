 ...
 */
/**
 * The newest context reading in each of several conversations, in one query.
 *
 * Occupancy is written onto the assistant message a turn produced (see
 * `attachUsageToLatestAssistantMessage`), so the chat list — which holds no
 * messages at all — cannot say how full a chat is without being told. This
 * tells it, for a page of chats at once.
 *
 * The `LIKE` is a PREFILTER, not the test. It narrows to rows that could carry
 * a reading so the window function is not ranking every message ever sent; the
 * parse below is what actually decides. Matching our own serialized shape,
 * not prose — and a false positive costs a parse, never a wrong answer.
 *
 * Returns nothing for a conversation whose turns never reported one, which is
 * how a chat gets no percentage rather than a guessed one.
 */
export async function getLatestContextPerConversation(
  conversationIds: readonly string[]
): Promise<Map<string, { tokens: number; window: number | null }>> {
  const out = new Map<string, { tokens: number; window: number | null }>();
  if (conversationIds.length === 0) return out;

  const placeholders = conversationIds.map((_, i) => `$${String(i + 1)}`).join(', ');
  try {
    const result = await pool.query<Pick<MessageRow, 'conversation_id' | 'metadata'>>(
      `SELECT conversation_id, metadata FROM (
    ...
    for (const row of result.rows) {
      ...
      try {
        ...
        // One unreadable row is one missing gauge, not a failed list.
        continue;
      }
    }
  } catch (error) {
    // Non-throwing: this decorates a list that must still render without it.
    getLog().warn({ err: error as Error }, 'db.latest_context_per_conversation_failed');
  }
  return out;
}
