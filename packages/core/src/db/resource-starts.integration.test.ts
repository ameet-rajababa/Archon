...
// Every case below points ARCHON_HOME at a fresh temp directory and seeds fixed
// user/codebase/conversation ids into it. Inside a container `getArchonHome()`
// ignores that and returns /.archon, so the seed landed in the operator's real
// database and every run after the first failed on its own leftover row.
honorArchonHomeEnv();
