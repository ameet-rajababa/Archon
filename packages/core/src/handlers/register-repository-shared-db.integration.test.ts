...
// The whole premise is that a managed path resolves under THIS test's ARCHON_HOME.
// Inside a container `getArchonHome()` ignores it and returns /.archon, so both
// hosts resolved to the operator's real home and the refusal never triggered.
honorArchonHomeEnv();
