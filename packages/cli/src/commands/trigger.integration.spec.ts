...
async function runCli(
  ...
  const child = Bun.spawn([process.execPath, cliPath, ...args], {
    ...
    env: {
      ...process.env,
      // Without this the child's own `getArchonHome()` ignores ARCHON_HOME inside a
      // container and opens /.archon, so no database ever appears at the scratch
      // path this spec then reads. It supersedes the bare DATABASE_URL clear.
