...
import { FilesV2Page } from './FilesV2Page';
...
function render(search: string): string {
  return renderToStaticMarkup(
    <MemoryRouter initialEntries={[`/console/p/${PROJECT}/files-v2${search}`]}>
      <Routes>
        <Route path="/console/p/:projectId/files-v2" element={<FilesV2Page />} />
      </Routes>
    </MemoryRouter>
  );
}

describe('FilesV2Page', () => {
  afterEach(() => {
    invalidate('files:');
    invalidate('file:');
  });
