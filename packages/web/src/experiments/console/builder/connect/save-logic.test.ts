...
describe('isReadOnlySource / saveTargetFor', () => {
  ...
  // #2578 — the helpers accept `string` precisely so an unrecognized wire value
  ...
  test('an unrecognized source is editable in place, not read-only (#2578)', () => {
    expect(isReadOnlySource('something-new')).toBe(false);
  });

  test('an unrecognized source saves to project scope, the most permissive target (#2578)', () => {
