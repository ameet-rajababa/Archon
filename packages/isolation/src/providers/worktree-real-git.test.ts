...
describe('WorktreeProvider against real git', () => {
  ...
  // The cases below point ARCHON_HOME at a temp directory and expect every managed
  // worktree under it. Inside a container `getArchonHome()` ignores that and returns
  // /.archon, so git was asked about a path in the operator's real workspace tree.
  honorArchonHomeEnv();
