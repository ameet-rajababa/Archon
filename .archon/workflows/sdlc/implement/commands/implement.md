...
This run must already be on that pull request's branch, or for a pull request from a fork, on the run's review branch at the pull request's head — launched onto it with `--adopt`, or from the pull request itself.

The repository that pull request belongs to is already resolved, deterministically, from this branch's own push configuration. Take it as given:

- `PR_REPO` is **$pr-target.output.repo** (`owner/repo`) — the repository to read the pull request from, and the same value for every `gh --repo` argument in this run. Empty when this checkout did not settle which repository the branch belongs to.
- `PR_REPO_BLOCKER` is **$pr-target.output.reason** — why it could not be settled, and empty when `PR_REPO` holds a repository.

Never work the repository out from a remote's name. In a fork-style checkout `origin` is the upstream, so a guard that read it would compare this branch against a pull request in a repository nobody chose — or against no pull request at all, which proves nothing while looking like a passed check. Never pass a raw remote URL to a command or put one in your report; it can carry a credential. An empty `PR_REPO` means the guard cannot be performed, so the work cannot start: stop with `done: true, green: false`, giving `PR_REPO_BLOCKER` as the blocker, and make no edits.

Confirm the branch before editing with `gh pr view <number> --repo "$PR_REPO" --json headRefName,headRefOid,isCrossRepository,maintainerCanModify`. For a same-repository pull request, `git branch --show-current` must equal `headRefName`. For a fork (`isCrossRepository: true`), `git rev-parse HEAD` must equal `headRefOid` at the start of the run, and `maintainerCanModify` must be true: that flag is the author's permission for this run to push to their branch, and without it the work has nowhere to go. If either check fails, stop with `done: true, green: false` and say which check failed; never switch branches, fetch the pull request's head into this checkout, or start a replacement branch — the run's estate is fixed at launch, and work done anywhere else is invisible to it. On a match, work and commit on the current branch as usual; the pull request stays the one public surface, and later nodes push to its head and read it back by its number.
...
A work item whose deliverable is a failing assertion is a different case, and it has its own word. When the item asks you to pin a finding by letting a new assertion fail — because correcting the code widens a gate, or is otherwise a decision only the operator may make — build it, commit it, and declare `done: true, green: false, red_cause: deliberate`, with the failing assertion and the decision it is waiting on in `summary`. Say the true thing: do not call intended red `introduced`, and do not quietly edit the literal to get green.

Then expect to be refused. Delivery carries no red to a pull request, so `deliberate` fails its gates exactly like any other red — the difference is that the refusal answers what you actually declared and names the routes that keep the finding: an expected-failure marker, which leaves it pinned and re-run while the suite stays green, or the operator's decision raised as its own item. The work is committed on the branch either way, which is what the operator reads.
...
- `red_cause` — why the verdict is not green. When a check you ran failed: `introduced`, your change caused it; `inherited`, the same check was already failing at this run's starting commit; `environment`, the machine this run is on caused it, not any code — a database or port a parallel process holds, a missing credential, a network fault. `incomplete` when you validated the work, no check you ran failed, but not every applicable check ran — you were stopped partway (a usage limit, a killed process). A check that ran and failed takes its own cause even when others never ran. `deliberate` when the failing check is the deliverable — an assertion you wrote to fail because the failure is the finding the work item asked for, and correcting the code is a decision only the operator may make. Always declared: use the empty string `""` when `green` is true, and when you are declaring a blocker on the work itself without having run checks at all
- `summary` — a few sentences: what you did, what stands, what (if anything) blocks. For `incomplete`, what stopped validation and which checks ran and passed. For `deliberate`, which assertion fails, what drift it pins, and why the correction is not yours to make
