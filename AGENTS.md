...
- **Deploying is asked for, and it does not answer.** From the container checkout, `bash scripts/request-deploy.sh` writes one file naming HEAD; a systemd `.path` unit on the host (`scripts/deploy-units/`) runs `scripts/deploy-local.sh`, which needs docker and sudo that the container does not have. The script returns immediately and the host then builds for minutes, so **the verdict is `/.archon/deploy-last.log`, not the exit code you just got**. A request already pending is refused rather than queued.
- **A deploy waits for a moment when the box holds nothing**, draining rather than interrupting: a turn in flight, a message queued behind one, and a workflow run that would come back as a `running` row nobody finishes are all things a container swap destroys. Keep talking to a chat and the wait keeps waiting. An open chat is not one of those — its provider session id is persisted, so it resumes with its context intact.
- **Nothing is deployed until something says which commit is running.** `deploy-local.sh` ends by asking the running container what it was built from, because every earlier step can succeed while doing nothing — a push never run, a pull blocked by git's ownership guard, a build from a checkout that had not moved. Treat "the merge landed" as not yet deployed.
